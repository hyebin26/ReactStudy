...
화살표 함수안에서 mutating 상태를 변경하면이 규칙이 위반되고 오류가 발생한다. 이것은 상태와 함수의 호출은 값을 리턴할 수 있고 Immer는 복제의 시도와 새로운 리턴된 값을 둘다 보지만 결과로 사용되는 값을 모르기 때문이다. 약간의 잠재적인 해결책은 값을 리턴하는 것을 가지는 것을 스킵하기 위해 void키워드를 사용하거나 중괄호를 사용하여 화살표 함수에 본문을 제공하고 리턴 값은 제공하지 않는다.
...
여기에 문제가 있다. Immer는 상태에 새로 삽입 된 개체를 래핑하지 않는다. 대부분의 경우 이것은 중요하지 않지만 값을 삽입하고 다음 추가 업데이트를 수행하려는 경우가있을 수 있다.
